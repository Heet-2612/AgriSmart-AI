...
export async function predictDisease(file: File): Promise<PredictionResponse> {
  ...
  try {
    ...
    throw new ApiError(0, 'Unable to connect to the diagnosis server. Please check your network connection.');
  }
  ...
  if (!res.ok) {
    ...
    throw new ApiError(res.status, message, errorData?.status);
  }
...
export async function recommendCrop(
  ...
  try {
    ...
    throw new ApiError(
      0,
      'Unable to connect to the recommendation server. Please check your network connection.'
    );
  }
...
export async function getWeather(location: string): Promise<WeatherResponse> {
  let res: Response;
  try {
    res = await fetch(`/api/weather?location=${encodeURIComponent(location)}`);
  } catch {
    throw new ApiError(0, 'Unable to connect to the weather service. Please check your network connection.');
  }

  if (!res.ok) {
    const errorData = await res.json().catch(() => ({ detail: res.statusText }));
    const message = extractErrorMessage(errorData, res.status, `Weather fetch failed with status: ${res.status}`);
    throw new ApiError(res.status, message);
  }
...
export async function getSeasonalClimate(
  ...
  let res: Response;
  try {
    res = await fetch(`/api/seasonal-climate?${params.toString()}`);
  } catch {
    throw new ApiError(0, 'Unable to connect to the climate service. Please check your network connection.');
  }

  if (!res.ok) {
    const errorData = await res.json().catch(() => ({ detail: res.statusText }));
    const message = extractErrorMessage(errorData, res.status, `Climate fetch failed with status: ${res.status}`);
    throw new ApiError(res.status, message);
  }
